...
case class WriterStartUpConfiguration(appConfig: File) extends AnyVal
...
object WriterStartUpConfiguration {
  ...
  private val parser = new scopt.OptionParser[RawStartUpConfiguration]("veresk") {
    head("Writer for Testing Mapr S3")
    opt[File]("app-config") valueName "<file-path>" required () action { (p, input) =>
  ...
  def parse(input: Seq[String]): Option[WriterStartUpConfiguration] =
    parser.parse(input, Default)    match {
      case Some(RawStartUpConfiguration(Some(a))) => Some(WriterStartUpConfiguration(a))
      case _                                      => None
    }
}
